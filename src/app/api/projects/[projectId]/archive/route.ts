...
export async function POST(request: Request, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  if (!body || typeof body !== "object" || Array.isArray(body) || Object.keys(body).length !== 2 || typeof (body as { expectedProjectVersion?: unknown }).expectedProjectVersion !== "number" || typeof (body as { reason?: unknown }).reason !== "string" || !key) return projectError("INVALID_INPUT", "Enter a valid project version and try again.", 400);
  try { return projectResponse(await archiveProject(user, (await params).projectId, { ...(body as { expectedProjectVersion: number; reason: string }), key })); } catch (error) { return projectFailure(error); }
}
